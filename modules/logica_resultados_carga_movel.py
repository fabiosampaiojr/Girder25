# ============================================================================
# Girder25 - logica_resultados_carga_movel.py
# Autor: Fábio Henrique Sampaio Júnior
# Controlador das janelas de resultados de envoltória e reações (carga móvel).
# ============================================================================
...
from modules.Calculadora_Carga_Movel import ativar_interatividade_envoltoria
...
class LogicaJanelaResultadosEnvoltoria(QDialog, Ui_janela_resultados_esforcos):
    """
    Janela de visualização de envoltórias de esforços (cortante ou momento fletor)
    para carga móvel. Permite filtrar por passo, destacar extremos e exportar.
    """

    def __init__(
        self, titulo_janela: str, titulo_diagrama: str, titulo_tabela: str,
        dados_tabela: list, figura_matplotlib, valores_destaque_min: list, valores_destaque_max: list
    ):
        super().__init__()
        self.setupUi(self)
        ...
        self._valores_min = valores_destaque_min
        self._valores_max = valores_destaque_max
    ...
    def _renderizar_figura(self):
        ...
        ativar_interatividade_envoltoria(self.fig_atual, canvas)
    ...
    def _obter_secoes_criticas(self, cabecalho, linhas, tipo_janela):
        ...
        is_movel = any("min" in c.lower() or "max" in c.lower() for c in cabecalho)

        if "Cortante" in tipo_janela:
        ...
        linha_max = max(linhas, key=lambda l: float(l[idx_max]))
        linha_min = min(linhas, key=lambda l: float(l[idx_min]))

        return {
            "Máximo": (f"({float(linha_max[idx_pos]):.2f} m)", float(linha_max[idx_min]), float(linha_max[idx_max])),
            "Mínimo": (f"({float(linha_min[idx_pos]):.2f} m)", float(linha_min[idx_min]), float(linha_min[idx_max]))
        }
    ...
    def _atualizar_tabela(self):
        ...
        linhas_filtradas.sort(key=lambda l: float(l[idx_pos]))

        val_max_filt = max((float(l[idx_max]) for l in linhas_filtradas), default=None)
        val_min_filt = min((float(l[idx_min]) for l in linhas_filtradas), default=None)
        ...
        cor_max = QColor("#81c784")
        cor_min = QColor("#e57373")

        for row_idx, linha in enumerate(linhas_filtradas):
            eh_max = (float(linha[idx_max]) == val_max_filt) if val_max_filt is not None else False
            eh_min = (float(linha[idx_min]) == val_min_filt) if val_min_filt is not None else False

            for col_idx, valor in enumerate(linha):
                if col_idx == idx_pos:
                    texto = f"{float(valor):.2f}"
                elif isinstance(valor, (int, float)):
                    texto = f"{int(round(float(valor)))}" if arredondar else f"{float(valor):.3f}"
                else:
                    texto = str(valor)
                ...
                if destacar:
                    if eh_max:
                        item.setBackground(cor_max)
                        item.setForeground(QColor("black"))
                        font = item.font()
                        font.setBold(True)
                        item.setFont(font)
                    elif eh_min:
                        item.setBackground(cor_min)
                        item.setForeground(QColor("black"))
                        font = item.font()
                        font.setBold(True)
                        item.setFont(font)
        ...
        html = gerar_html_resultados_esforcos_calculos(secoes_criticas, "movel", tipo_janela.lower())
...
class LogicaJanelaResultadosReacoesCargaMovel(QDialog, Ui_janela_resultados_reacoes):
    """
    Janela de visualização detalhada das reações de apoio para carga móvel.
    Permite destacar valores extremos e exportar a tabela.
    """

    def __init__(self, titulo_janela: str, dados_tabela: list, valores_destaque_max: list):
        super().__init__()
        self.setupUi(self)
        self.setWindowTitle(titulo_janela)
        self.dados_tabela = dados_tabela
    ...
    def _obter_secoes_criticas(self, cabecalho, linhas):
        ...
        return {
            "Máximo": (f"Apoio {linha_max[idx_nome]} ({float(linha_max[idx_pos]):.2f} m)", float(linha_max[idx_min]), float(linha_max[idx_max])),
            "Mínimo": (f"Apoio {linha_min[idx_nome]} ({float(linha_min[idx_pos]):.2f} m)", float(linha_min[idx_min]), float(linha_min[idx_max]))
        }
    ...
    def _atualizar_tabela(self):
        ...
        linhas_todas.sort(key=lambda l: float(l[idx_pos]))
        posicoes = [float(linha[idx_pos]) for linha in linhas_todas]
        n = len(linhas_todas)

        val_max = max((float(l[idx_max]) for l in linhas_todas), default=None)
        val_min = min((float(l[idx_min]) for l in linhas_todas), default=None)
        ...
        for row_idx, linha in enumerate(linhas_todas):
            eh_max = (float(linha[idx_max]) == val_max) if val_max is not None else False
            eh_min = (float(linha[idx_min]) == val_min) if val_min is not None else False
            ...
            for col_idx, valor in enumerate(linha):
                ...
                if destacar:
                    if eh_max:
                        item.setForeground(cor_max)
                        font = item.font()
                        font.setBold(True)
                        item.setFont(font)
                    elif eh_min:
                        item.setForeground(cor_min)
                        font = item.font()
                        font.setBold(True)
                        item.setFont(font)
        ...
        html = gerar_html_resultados_esforcos_calculos(secoes_criticas, "movel", "reacoes")
        self.textEdit_resumo.setHtml(html)
    ...
    def abrir_manual(self):
        ...
        viewer.display_page(84)   # página 85 do manual → índice 84
        viewer.exec()